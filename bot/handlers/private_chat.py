...
class PrivateHandler(bot_utils.BaseHandler):
    @staticmethod
    @tgbot.message_handler(TgUser.is_private,
                           commands=keyboards.Start.commands())
        ...
        if len(parts) == 2:
            ...
            try:
                ...
                if s.startswith('token:'):
                    ...
                    if len(p) == 2:
                        token_id = p[1]
                        t = Token.objects.filter(id=token_id,
    ...
    def unauthorized(cls, tguser: TgUser):
        ...
        tguser.render_to_string('bot/private/errors/not_authorized.html',
                                context=dict(url=url), edit=True)

    @staticmethod
    @tgbot.message_handler(TgUser.is_private, TgUser.is_authorized,
                           regexp=keyboards.Boards.emoji_to_regexp())
    @tgbot.message_handler(TgUser.is_private, TgUser.is_authorized,
                           commands=keyboards.Boards.commands())
    def boards(tguser: TgUser):
        ...
        tguser.render_to_string('bot/private/choose_board.html',
                                keyboard=keyboards.Boards(tguser, boards,
                                                          timer_board_ids),
                                edit=True)

    @staticmethod
    @tgbot.callback_query_handler(TgUser.is_authorized,
                                  data_startswith='/board ')
    def board(tguser: TgUser, board_id=None):
        ...
        tguser.render_to_string('bot/private/choose_list.html',
                                keyboard=keyboards.Lists(tguser, lists,
                                                         timer_list_ids),
                                edit=True)

    @staticmethod
    @tgbot.callback_query_handler(TgUser.is_authorized,
                                  data_startswith='/board_list ')
    def board_list(tguser: TgUser, list_id=None):
        ...
        tguser.render_to_string('bot/private/choose_card.html',
                                keyboard=keyboards.Cards(tguser, list_id,
                                                         cards,
                                                         timer_card_ids),
                                edit=True)
    ...
    def card(tguser: TgUser):
        ...
        message = tguser.render_to_string('bot/private/show_card.html',
                                          ...
                                          keyboard=keyboards.Card(tguser,
                                                                  card_id,
                                                                  timer),
                                          edit=True)
        if timer:
    ...
    def timer(tguser: TgUser):
        ...
        timer = tguser.timer_set.filter(card_id=card_id).first()
        if not timer:
            tguser.answer_callback_query('Timer was not started',
                                         show_alert=True)
    ...
    @staticmethod
    @tgbot.callback_query_handler(TgUser.is_authorized,
                                  data_startswith='/timer_add ')
    def timer_add(tguser: TgUser):

        time = tguser.callback_query_data_get(1, as_int=True)
        ...
        if not timer:
            tguser.answer_callback_query('Timer was not started',
                                         show_alert=True)
            raise bot_utils.StateErrorHandler('timer_not_started')
        timer.created_at -= timedelta(minutes=time)
        timer.save()
        tguser.edit_message_reply_markup(
            keyboard=keyboards.Card(tguser, card_id, timer))
    ...
    @staticmethod
    @tgbot.message_handler(TgUser.is_private,
                           regexp=keyboards.Help.emoji_to_regexp())
    @tgbot.message_handler(TgUser.is_private,
                           commands=keyboards.Help.commands() + ['sos'])
    def help(tguser: TgUser):
        tguser.render_to_string('bot/private/help.html',
                                keyboard=keyboards.Start)
